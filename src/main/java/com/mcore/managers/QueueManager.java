...
public class QueueManager {
    ...
    public void cleanup(Player player) { leave(player); }
    ...
    private void playMusic(Player p) {
        ...
        if (soundName != null && !soundName.isEmpty()) {
            try {
                p.playSound(p.getLocation(), Sound.valueOf(soundName.toUpperCase()), 10f, 1f);
    ...
    private void stopMusic(Player p) {
        ...
        if (soundName != null && !soundName.isEmpty()) {
            try {
                p.stopSound(Sound.valueOf(soundName.toUpperCase()));
            } catch (Exception ignored) {}
        }
    }
